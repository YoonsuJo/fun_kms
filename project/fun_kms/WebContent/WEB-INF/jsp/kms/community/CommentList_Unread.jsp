...
<input name="ajaxMode" type="hidden" value="1">
				...
				<tr>
					...
					<td class="date">
						...
						<c:if test="${result.frstRegisterId == user.stringNo}">
							<span class="btn_plus">
								<input type="hidden" id="commentCn_${result.commentNo}" value="<c:out value='${result.commentCnPrint}' escapeXml='false'/>" >
								<a href="javascript:fn_egov_selectCommentForupdt('<c:out value="${result.commentNo}" />', '<c:out value="${status.index}" />', this);"><img src="${imagePath}/btn/btn_plus02.gif" /></a>
			...
			<td class="last pL10">
				<c:choose>
					<c:when test="${searchVO.commentNo == ''}">
						<img id="registComment" class="cursorPointer" src="${imagePath}/btn/btn_regist02.gif"/>
					</c:when>
					<c:otherwise>
						<img id="registComment" class="cursorPointer" src="${imagePath}/btn/btn_regist02.gif"/>
					</c:otherwise>
