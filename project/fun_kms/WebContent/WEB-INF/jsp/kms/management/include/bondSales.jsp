...
<div class="boardList02 mB20">
	...
	<thead>
		<tr>
			...
			<th>채권관리</th>
			<th>효력</th>
			<th class="td_last">세금계산서<br/>발행여부</th>
		</tr>
	</thead>
	<tbody>
		<c:choose>
				...
				<c:forEach items="${resultList}" var="result" varStatus="c">
					<tr>
						...
						<td class="td_last txt_center">
							<c:if test="${result.mapYn=='Y'}">
								<span id="span_pub_${result.docId}">발행</span>
								<c:if test="${user.isAdmin == 'Y' }">
									<input type="button" class="btn_gray fr" value="보기" onclick="showLayer('bondPrjLayer', '${result.docId}', this, 'taxShow')"/>
								</c:if>
							</c:if>
							<c:if test="${result.mapYn=='N'}">
								<span id="span_pub_${result.docId}"></span>
								<c:if test="${user.isAdmin == 'Y' }">
									<input type="button" class="btn_gray fr" value="처리" onclick="showLayer('bondPrjLayer', '${result.docId}', this, 'taxList')"/>
								</c:if>
							</c:if>
						</td>
					</tr>
