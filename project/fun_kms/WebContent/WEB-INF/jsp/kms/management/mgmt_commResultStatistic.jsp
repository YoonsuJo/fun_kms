...
$(document).ready(function() {
	...
	if (recalcYn == 'Y') {
		<c:forEach items="${resultList}" var="result" varStatus="c">
			$.post("/ajax/commLaborResultStatistic.do?searchYear=${searchVO.searchYear}&searchMonth=${searchVO.searchMonth}&sectorNo=${result.sectorNo}",
				function(data){
...
<body><div id="pop_regist05" style="height:auto;border-bottom:0px">
 	<div id="pop_top">
 	   <ul>
			<li><img src="../images/inc/pop_bullet.gif" /></li>
			<li class="popTitle">공통비 운영 실적분석</li>
		</ul>
 	</div>
 	<div class="pop_con0">
 	
 		<!-- 상단 검색창 시작 -->
		<form name="frm" action="${rootPath}/management/commResultStatistic.do" method="POST" onsubmit="search(0); return false;">
        ...
        <caption>공통비 운영 실적분석 보기</caption>
        <colgroup>
        	<col class="col150" />
        	<col width="93px" />
        	<col width="93px" />
        ...
        <tbody>
        	<c:forEach items="${resultList}" var="result" varStatus="c">
	        	<tr <c:if test="${result.colorTyp == 'A'}">class="TrBg5"</c:if><c:if test="${result.colorTyp == 'B'}">class="TrBg4"</c:if>>
		         	<td class="pL10 T13B border-black">${result.sectorNm}</td>
