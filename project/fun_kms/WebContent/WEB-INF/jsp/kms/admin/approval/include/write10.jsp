...
function prjSelect(prjNmObj, prjIdObj,docId){
	$.post("/ajax/approval/selectSaelsPurchaseOutList.do?prjId="+prjIdObj.val(),function(data){
		...
		if(data.length>=1)
		{
			...
			$.each(data,function(idx,elm){
				var option = $('<option value="'+elm.docId+'">'+
					...
					'(' + '매출액 : ' +jsAddComma(elm.salesSum) + '원)'+  '</option>');
		...
		{
			selector.html('<option value="" label="프로젝트를 선택하세요."/>');			
		}
	});
};
</script>
	...
	<tbody>
		<tr <c:if test="${approvalVO.templtId!=10}">style="display:none;"</c:if>>
			<td class="pL10 pT5 pB5" colspan="6">
				<input type="radio" name="expense[${status.index}].isPreset" value="N" <c:if test="${empty result.presetId}">checked</c:if>> 
				직접입력 <span class="pL7"></span> 
				<input name="expense[${status.index}].isPreset" type="radio" value="Y" <c:if test="${not empty result.presetId}">checked</c:if>/> 
				빠른입력 <span class="pL7"></span> 
				<select name="expense[${status.index}].presetIdSelector" class="span_9 <c:if test="${empty result.presetId}">hidden</c:if>" 
				<c:if test="${empty result.presetId}">disabled</c:if>>
		...
		<tr>
			...
			<td class="pL10" colspan="5">
				<label>
					...
					<c:if test="${result.expSpendTyp=='PP' }">checked</c:if> <c:if test="${result.expSpendTyp=='CC' }">disabled</c:if>/>
					개인결제 
				</label>
				<label>
					<input name="expense[${status.index}].expSpendTyp" type="radio" value="CP" 
					<c:if test="${result.expSpendTyp=='CP' }">checked</c:if> <c:if test="${result.expSpendTyp=='CC' }">disabled</c:if>/>
					회사결제 
				</label>
					<label>
					<input name="expense[${status.index}].expSpendTyp" type="radio" value="CC" 
					<c:if test="${result.expSpendTyp=='CC' }">checked</c:if> <c:if test="${result.expSpendTyp=='CC' }">disabled</c:if>/>
					법인카드 
				</label>
				<label>
					<input name="expense[${status.index}].expSpendTyp" type="radio" value="KK" 
					<c:if test="${result.expSpendTyp=='KK' }">checked</c:if> <c:if test="${result.expSpendTyp=='CC' }">disabled</c:if>/>
					체크카드 
				</label>
				...
				<td class="pL10" colspan="5" > 
					<ul style="width : 100%;">
						...
						<select name="expense[${status.index}].column2">
							<option value="" label="프로젝트를 선택하세요.">
							</option>
						</select>
						</li>
					</ul>
...
<!-- 버튼 시작 -->
<div class="btn_area04"><img src="${imagePath}/btn/btn_add.gif"
