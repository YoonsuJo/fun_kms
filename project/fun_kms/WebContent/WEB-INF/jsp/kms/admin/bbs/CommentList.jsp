...
function fn_egov_insert_commentList() {
	...
	document.frm.action = "<c:url value='${rootPath}/admin/insertComment.do'/>";
	document.frm.submit();
}
...
function fn_egov_updt_commentList() {
	...
	document.frm.action = "<c:url value='${rootPath}/admin/updateComment.do'/>";
	document.frm.submit();
}

function fn_egov_selectCommentForupdt(commentNo, index) {
	document.frm.commentNo.value = commentNo;
	document.frm.action = "<c:url value='${rootPath}/admin/selectBoardArticle.do'/>";
	document.frm.submit();
}
...
function fn_egov_deleteCommentList(commentNo, index) {
	if (confirm('<spring:message code="common.delete.msg" />')) {
		document.frm.modified.value = "true";
		document.frm.commentNo.value = commentNo;
		document.frm.action = "<c:url value='${rootPath}/admin/deleteComment.do'/>";
		document.frm.submit();
	}
}
...
function fn_egov_select_commentList(pageNo) {
	...
	document.frm.action = "<c:url value='${rootPath}/community/selectBoardArticle.do'/>";
	document.frm.submit();
}

function fn_egov_resize_commentCn(button) {
	var size = parseInt(document.frm.commentCn.style.height);
	if (isNaN(size))
		size = 35;
	...
	if (button == 'up') {
		if (size > 35) {
			size = size - 100;
			document.frm.commentCn.style.height = size + "px";
		}
	} else {
		size = size + 100;
		document.frm.commentCn.style.height = size + "px";
