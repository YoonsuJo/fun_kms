...
//TENY_170218  목록을 보고자 하려고 할때 
function searchInvoiceList() {
	document.invoiceVOFm.action = '<c:url value="${rootPath}/fund/invoiceList.do?" />';	
	document.invoiceVOFm.submit();
}
...
<div id="pop_reg_div01">
 	...
 	<div class="pop_reg_div02">
		...
		<br/><div class="btn_area">           		    	
			...
			<!-- 권한 : 발행요청자 -->
			<!-- 상태 : 미발행 상태. -->
			<c:if test="${(invoiceVO.writeUserNo == user.no && invoiceVO.status == 2)}">
			...
			<!-- 기능 : 세금계산서 발행요청서 목록 페이지로 이동할때 사용하는 기능 -->
			<img src="../../images/btn/btn_list.gif" onclick="searchInvoiceList();" class="cursorPointer"/>
		</div>
		<!-- 버튼 끝 -->
