...
function editeComment() {
	...
	document.commentFrm.action = "${rootPath}/mobile/cooperation/updateBusinessContactComment.do";
...
<input type="hidden" name="bcId" value="${bcComment.bcId}"/>
<input type="hidden" name="no" value="${bcComment.no}"/>
<textarea name="bcCommentCn" placeholder="내용을 입력해 주세요." rows="6" cols="85">${bcComment.bcCommentCn}</textarea>
</form>
</div>

<p>
<a href="javascript:goBack();"><button class="bgBtn shadowBtn btnreply" style="float:left; margin-left:10px; width:50px">취소</button></a>
<a href="javascript:editeComment();"><button class="bgBtn shadowBtn btnreply" style="float:right; margin-right:10px; width:50px">수정</button></a>
</p>
<br/>
<!-- E:콘텐츠 들어가는곳 -->
</div>
