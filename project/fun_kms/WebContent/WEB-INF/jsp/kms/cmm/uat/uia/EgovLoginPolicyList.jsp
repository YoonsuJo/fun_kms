...
function fncSelectLoginPolicyList(pageNo){
    ...
    document.listForm.action = "<c:url value='/com/uat/uia/selectLoginPolicyList.do'/>";
    document.listForm.submit();
}

function fncSelectLoginPolicy(emplyrId) {
    document.listForm.emplyrId.value = emplyrId;
    document.listForm.action = "<c:url value='/com/uat/uia/getLoginPolicy.do'/>";
    document.listForm.submit();    
}
...
function fncAddLoginPolicyInsert() {
    ...
    if(fncInsertCheckId()) {
        document.listForm.action = "<c:url value='/com/uat/uia/addLoginPolicyView.do'/>";
        document.listForm.submit();
    }
}
...
function fncLoginPolicyListDelete() {
	if(fncManageChecked()) {
        if(confirm("삭제하시겠습니까?")) {
            document.listForm.action = "<c:url value='/com/uat/uia/removeLoginPolicyList.do'/>";
            document.listForm.submit();
        }
    }
}
...
<form name="listForm" action="<c:url value='/com/uat/uia/selectLoginPolicyList.do'/>" method="post">
<table width="100%" cellpadding="8" class="table-search" border="0">
    <tr>
        <td width="25%"class="title_left">
            <h1><img alt="" src="<c:url value='/images/egovframework/cmm/uat/uia/icon/tit_icon.gif' />" width="16" height="16" hspace="3" align="middle" >&nbsp;로그인정책 관리</h1></td>
        <td width="55%">사용자 명 : <input name="searchKeyword" type="text" value="<c:out value="${loginPolicyVO.searchKeyword}"/>" size="25" title="검색" onkeypress="press();" ></td>
        <th width="20%" align="right">
            <table border="0" cellspacing="0" cellpadding="0">
                <tr> 
                    <td><span class="button"><input type="submit" value="<spring:message code="button.inquire" />" onclick="fncSelectLoginPolicyList('1'); return false;"></span></td>
                </tr>
            </table>
        </th>  
    </tr>
 ...
 <c:forEach var="loginPolicy" items="${loginPolicyList}" varStatus="status">
  <tr>
    <td class="lt_text3" nowrap>
        <form name="item" method="post" action="<c:url value='/com/uat/uia/getLoginPolicy.do'/>">
