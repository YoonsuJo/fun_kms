		...
		<tbody>
			<tr>
				...
				<input disabled="disabled" name="expense[${status.index}].presetId" type="radio" 
					<c:if test="${not empty result.presetId }"> checked</c:if> /> 빠른입력
					<c:if test="${not empty result.presetId }"><span class="presetNm">- ${result.presetNm }</span></c:if>
				</td>
			</tr>
			...
			<c:if test="${appTyp.templtId==13}">
				<tr class="salesInfoTr">
					...
					<td class="pL10" colspan="5" > 
					<span class="column1">
						<c:if test="${user.admin}"><a href="/admin/approval/eappExpU.do?expId=${result.expId}"></c:if>
						<c:choose>
							<c:when test="${result.column1==1}">외부매입</c:when>
							<c:when test="${result.column1==2}">내부매입</c:when>
						</c:choose>
						<c:if test="${user.admin}"></a></c:if>
					</span>
					</td>
				</tr>
				<tr class="salesInfoTr">
			...
			<tr>
				...
				<td class="pL10"  colspan="5">
			 	...
			 	<span class="corporateCardS <c:if test="${result.expSpendTyp!='CC'}">hidden</c:if>">
			 		<span class="pL20"></span>카드번호 : 
			 		<span class="txt_red cardId"> <c:if test="${result.cardId==NULL}">카드번호 미선택 (확인필요)</c:if></span>
			 		<span class="txt_blue cardId">${result.cardId }</span>
			 	</span>
				</td>
			</tr>
			<tr>
