   ...
   <table border="0" cellspacing="0" cellpadding="0">
    <tr> 
      ...
      <c:if test="${brdMstrVO.authFlag == 'Y'}">
	      <td><img src="<c:out value="${ImgUrl}"/>btn/bu2_left.gif" alt="버튼이미지" title="버튼이미지" width="8" height="20"></td>
	      <td class="btnBackground" nowrap><a href="<c:url value='/cop/bbs${prefix}/addBoardArticle.do'/>"><spring:message code="button.create" /></a></td>
	      <td><img src="<c:out value="${ImgUrl}"/>btn/bu2_right.gif" alt="버튼이미지" title="버튼이미지" width="8" height="20"></td>
      </c:if>
    </tr>
   </table>	  			  
	    	...
	    	<c:choose>
	    		...
	    		<c:otherwise>
		    		<form name="subForm" method="post" action="<c:url value='/cop/bbs${prefix}/selectBoardArticle.do'/>">
						...
						<input type="hidden" name="authFlag" value="<c:out value='${brdMstrVO.authFlag}'/>" />
						<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>"/>
			    		<span class="link"><input type="submit" style="width:320px;border:solid 0px black;text-align:left;" value="<c:out value="${result.nttSj}"/>" onclick="javascript:fn_egov_inqire_notice('<c:out value="${result.nttId}"/>','<c:out value="${result.bbsId}"/>'); return false;"></span>
			    	</form>    
	    		</c:otherwise>
	    	</c:choose>
	    </td>
