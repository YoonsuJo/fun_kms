...
<script type="text/javascript">
	function fn_egov_select_scrapList(pageNo) {
		...
		document.frm.action = "<c:url value='/cop/bbs/selectScrapList.do'/>";
		document.frm.submit();
	}
	
	function fn_egov_delete_scrap() {		
		if (confirm('<spring:message code="common.delete.msg" />')) {
			document.frm.action = "<c:url value='/cop/bbs/deleteScrap.do'/>";
			document.frm.submit();
		}	
	}
	
	function fn_egov_moveUpdt_scrap() {
		document.frm.action = "/cop/bbs/forUpdateScrap.do";
		document.frm.submit();			
	}
</script>
...
<c:out value="${board.nttCn}" escapeXml="false" />	
	...
	<tr>
		<c:if test="${scrap.frstRegisterId == sessionUniqId}">
		  <td>  
		  	<form name="modifyFrm" method="post" action="<c:url value='/cop/bbs/forUpdateScrap.do'/>">
				<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>">
				<input type="hidden" name="bbsId" value="<c:out value='${scrap.bbsId}'/>" >
				<input type="hidden" name="nttId" value="<c:out value='${scrap.nttId}'/>" >
				<input type="hidden" name="scrapId" value="<c:out value='${scrap.scrapId}'/>" >
	      ...
	      <td width="10"></td>
		  <td>  
		  	<form name="deleteFrm" method="post" action="<c:url value='/cop/bbs/deleteScrap.do'/>">
				<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>">
				<input type="hidden" name="bbsId" value="<c:out value='${scrap.bbsId}'/>" >
				<input type="hidden" name="nttId" value="<c:out value='${scrap.nttId}'/>" >
				<input type="hidden" name="scrapId" value="<c:out value='${scrap.scrapId}'/>" >
      ...
      <td>
      	<form name="listFrm" method="post" action="<c:url value='/cop/bbs/selectScrapList.do'/>">
