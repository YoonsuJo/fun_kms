	...
	var TotalSales = function(target)
	{
	   ...
	   //Write...
		...
		this.modify = function (options)
		{
			...
			else if(options.deposit != globalData.deposit || options.maintenancePercent != globalData.maintenancePercent)
			{
	...
	var Labor = function(target, options)
    {
       ...
       var userListBasicTr = $('<tr>'+
    	...
    	this.init = function(typ) {
			...
			var applyBtn = $('<span class="th_plus03 mT10 mB10"><img src="' + imagePath + '/btn/btn_add03.gif" class="cursorPointer applyLaborUserListTableB"></span>');
        ...
        this.calcUserInputList = function()
        {
     	...
     	this.insertLaborUserList = function(userIdArray,userNmArray){
     		...
     		for (var i = 0; i < userIdArray.length ; i++)
			{
				...
				userListTr.find(".userListInputNmTd").html(userListTr.find(".userListInputNmTd").html() + '<input type="hidden" name="inputId"/>');
				...
				userListTr.find('[name=stDt]').val($('#salesDocWriteT').find('[name$=stDt]').val());
	     		...
	     		userListTr.find(".userTrDelB").click(function(){
					...
					var delUserId = "";
					var boolDel = true;
					var idxList = idxList = $('.LaborUserListTable').find('tr');
   ...
   var PurchaseOut = function(target, options)
	{
		...
		var basicTr = $('<tr>'+
				...
				'<td class="pL10" >'+
           		'<input type="text" name="cost" class="span_6 currency"  value="0" /> 원'+
           	'</td>'+
           	'<td class="title">매입월</td>'+
           	'<td class="pL10" >'+
           		'<input type="text" name="stDt" class="span_4 calGenMonth"  />'+
           		'<span class="hidden edDtSpan"> ~'+
           			'<input type="text" name="edDt" class="span_4 calGenMonth"  />'+
           		'</span>' +
           		'<input type="checkbox" name="isRepeat"/> 반복'+
           	'</td>'+
		...
		var viewTr = $('<tr>'+
				'<td class="title">금액</td>'+
				'<td class="pL10" >'+
					'<span class="span_6 currency cost"></span>원'+
				'</td>'+
				'<td class="title">매입월</td>'+
				'<td class="pL10" >'+
       ...
       this.addWriteTr = function (){
	       ...
	       addedTr.find('[name$=isRepeat]').click(function(){
	      		  ...
	      		  {
	      			  $(this).closest("tr").find(".edDtSpan").show();
	      		  }
	      		  else
	      			  $(this).closest("tr").find(".edDtSpan").hide();
	       });
        	 ...
        	 $.each(globalData.purchaseOutObj, function(key,val){
        		 ...
        		 if(val.identifyNo != identifyNo)
        		 {
        			 ...
        			 else
        			 {
        				 ...
        				 cloneTr.find('[name$=cost]').val(jsAddComma(costSum));
				 ...
				 costSum = val.cost;
				 if(stDt=="")
					 stDt = getYm(val.year,val.month);
	        	 edDt =getYm(val.year,val.month);
	        	 ct =val.ct;
        	 });	
        	 ...
        	 $.each(globalData.purchaseOutObj, function(key,val){
        		 if(val.identifyNo != identifyNo)
        		 {
        			 ...
        			 else
        			 {
        				 ...
        				 cloneTr.find('.cost').html(jsAddComma(costSum));
				 ...
				 costSum = val.cost;
				 if(stDt=="")
					 stDt = val.year +"."+ val.month;
	        	 edDt =val.year +"."+ val.month;
	        	 ct =val.ct;
        	 });	
        	 if(globalData.purchaseOutObj.length>=1)
        	 {
   ...
   var PurchaseInGeneral = function(target, options)
	{
       ...
       this.target = target;
		...
		var basicTr = $('<tr>'+
				'<td class="title">금액</td>'+
				'<td class="pL10" >'+
				'	<input type="text" name="cost" class="span_4 currency" value="0"/> 원'+ 
				'</td>'+
			...
			target.find('table').each(function(){
					...
					if(isInPeriod(year,month))
					{
						...
						if(sales[getYm(year,month)]!=0)
							$(this).find(".profitOnSalesPercent.month"+month).html(
									roundXL(profitOnSales[getYm(year,month)]*100/sales[getYm(year,month)],1)
							);
						else
							$(this).find(".profitOnSalesPercent.month"+month).html("-");
				...
				if(salesSum[year]!=0)
					$(this).find(".profitOnSalesPercent.horizonTotal").html(
							roundXL(profitOnSalesSum[year]*100/salesSum[year],1)
					);
				else
					$(this).find(".profitOnSalesPercent.horizonTotal").html("-");
